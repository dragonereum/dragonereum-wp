...
\vspace{\baselineskip}
where $XP_e$ are the Experience Points earned after a battle, $DS_{d}$\ \  is the DS of the defeated dragon, and $DS_{w}$ is the DS of the winning dragon. In the case when the attacking dragon loses, and the defending dragon wins, it will receive\  $XP_e/2$ and XP won is calculated according to the formulas below.\par 

\begin{center}
	\begin{equation}
	XP_e= \begin{cases}
	\left(\frac{DS_{d}}{DS_{w}}\right)^5 & \mbox{if } DS_{d} < DS_{w}\\
	\left(\frac{DS_{d}}{DS_{w}}\right) & \mbox{if } DS_{d} \geq DS_{w},
	\end{cases}
	\end{equation}
\end{center}
...
\vspace{\baselineskip}
\textit{w}here R is the \textit{battle winner reward}, G is the Gold balance in the game account, $I_{ds}$ was defined in Equation \ref{Eq:I_{ds}} above, and $f_{DS}$ is the \textit{dragon strength} \textit{factor} defined as,\par
