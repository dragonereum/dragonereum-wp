...
Our solution uses a block’s hash to generate random numbers. For complexity and the possibility to serve multiple players at the same time, we mix the previously generated random number, the block.timestamp and the hash of the previous block. So if similar transactions are included in one block, they will get different random numbers even though the hash of the block is the same,\par

\begin{Center}
\textit{uint256(keccak256(keccak256(block.blockhash(block.number - 1), \_seed), now));}
\end{Center}\par

where\tab \textit{\_seed}  is the previously generated random number, and\textit{ now} is the block.timestamp.\par
